...
describe Mercurial::Shell do
  ...
  it "should compile commands" do    
    ...
    Mercurial::Command.expects(:new).with("cd #{ @repository.path } && /usr/local/bin/hg log", :repository => @repository, :cache => nil).returns(command_mock).once
    @shell.hg('log')
  end
  
  it "should sanitize command arguments" do
    command_mock = mock('command', :execute => true)
    Mercurial::Command.expects(:new).with(%Q[ls 'file with nasty '\\''quote'], :repository => nil, :cache => nil).returns(command_mock).once
